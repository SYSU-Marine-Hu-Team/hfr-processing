# copied from https://github.com/teresaupdyke/qccodar/blob/main/src/qccodar/codarutils.py
#!/usr/bin/env python
#
""" CODAR Utilities
"""
...
def generate_radialshort(r, table_type='LLUV RDL7', numdegrees=3, weight_parameter='MP'):
    ...
    rs.data['HEAD'] = np.mod(rs.data['BEAR'] + 180., 360.)
    ...
    pts = np.array([geopy.distance.geodesic(kilometers=r).destination(origin, b)[0:2] for (r, b) in zip(rs.data['RNGE'], rs.data['BEAR'])])
